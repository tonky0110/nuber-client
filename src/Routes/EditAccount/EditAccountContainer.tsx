...
class EditAccountContainer extends React.Component<IProps, IState> {
  ...
  > = async event => {
    ...
    if (files) {
      ...
      formData.append("api_key", "148487887377268");
      formData.append("upload_preset", "kdqpizrk");
      formData.append("timestamp", String(Date.now() / 1000));
      const {
        data: { secure_url }
      } = await axios.post(
        "https://api.cloudinary.com/v1_1/tonky0110/image/upload",
        formData
      );
      if (secure_url) {
        this.setState({ uploading: false, profilePhoto: secure_url });
      }
    }
    this.setState({
      [name]: value
